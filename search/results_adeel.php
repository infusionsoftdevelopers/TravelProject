...
function findAirport($code, $airports) {
    // Defensive check: ensure $code is a string and not empty
    if (!is_string($code) || empty($code)) {
        return null;
    }
    
    foreach ($airports as $apt) {
        if (isset($apt['code']) && strcasecmp($apt['code'], $code) === 0) {
            return $apt;
        }
    }
    return null;
                                ...
                                <div class="flight-body">
                                    <div class="tickets-sub-area">
                                        ...
                                        <div class="flight-section">
                                            ...
                                            <div class="to">
                                                <?php $lastOutSeg = end($outSegs); ?>
                                                <div class="airport-code"><?php echo htmlspecialchars($lastOutSeg['to']); ?>
                                                </div>
                                                <div class="airport-name"><?php
                                                $toAirport = findAirport($lastOutSeg['to'], $AIRPORTS);
                                                echo $toAirport ? htmlspecialchars($toAirport['city']) : '';
                                                ?></div>
                                                <div class="flight-time">
                                                    <?php echo date('g:i A', strtotime($lastOutSeg['arrive'])); ?></div>
                                                <div class="flight-date">
                                                    <?php echo date('D d, M', strtotime($lastOutSeg['arrive'])); ?></div>
                                            </div>
                                            </div>
                                        ...
                                        <?php if ($res['inbound']): ?>
                                            <div class="flight-section">
                                                ...
                                                <div class="to">
                                                    <?php $lastInSeg = end($inSegs); ?>
                                                    <div class="airport-code"><?php echo htmlspecialchars($lastInSeg['to']); ?>
                                                    </div>
                                                    <div class="airport-name"><?php
                                                    $toAirport = findAirport($lastInSeg['to'], $AIRPORTS);
                                                    echo $toAirport ? htmlspecialchars($toAirport['city']) : '';
                                                    ?></div>
                                                    <div class="flight-time">
                                                        <?php echo date('g:i A', strtotime($lastInSeg['arrive'])); ?></div>
                                                    <div class="flight-date">
                                                        <?php echo date('D d, M', strtotime($lastInSeg['arrive'])); ?></div>
                                                </div>
                                                </div>
